...
import { AlertTriangle, BarChart3, Bell, CalendarDays, Check, ChevronDown, CircleHelp, Clock3, Contrast, Database, Download, FileJson, Layers3, MapPin, Menu, MoreHorizontal, Play, Plus, Route, Search, Sparkles, SunMoon, UsersRound, Wrench, X, Zap } from "lucide-react";
...
import { applyAverageWindows, fallbackTravel, idleOptimization, minutesLabel, scaleEngineers, scaleJobs, type Engineer, type Job, type OptimizationResult, type Region, type RoutePlan, type TravelMatrix } from "@/lib/vrptw";
  ...
  return <Dialog open={Boolean(job)} onOpenChange={open => !open && onClose()}>
    <DialogContent className="explain-dialog job-inspect-dialog">
      ...
      {job && <>
        ...
        <div className="job-baseline"><b>Жадный baseline</b><span>{baselineEngineer && baselinePlan ? `${baselineEngineer.name} · ${baselinePlan.stops.findIndex(stop => stop.jobId === job.id) + 1}-я остановка · ${formatDistance(baselinePlan.distanceKm)} за маршрут` : job.baselineUnassignedReason ?? "Без назначения"}</span></div>
        {engineer && plan ? <>
...
export default function Dashboard() {
  ...
  const [solverEngine, setSolverEngine] = useState<SolverEngine>("heuristic-browser");
...
const titles = { plan: ["План работ", started ? "17 августа 2026 · VRPTW по трём регионам" : "Задайте параметры справа и запустите построение маршрутов"], requests: ["Заявки", "CSV и срочные работы"], team: ["Инженеры", "Ресурсы и рассчитанная загрузка"], analytics: ["Аналитика", "Показатели текущего VRPTW-плана"] } as const; const distanceDataReady = csvMeta.geocoding.fallbackAddresses === 0 && (applied?.jobs ?? 0) <= sourceJobs.length && (applied?.engineers ?? 0) <= sourceEngineers.length && extraJobs.every(job => job.geocodeVerified === true) && !matrixFallback; const distanceReady = started && result.comparison.commonAssigned > 0 && result.comparison.distanceDeltaPercent != null && distanceDataReady; const distanceWarning = !started ? "Запустите построение маршрутов для сравнения." : !distanceDataReady ? ((applied?.jobs ?? 0) > sourceJobs.length || (applied?.engineers ?? 0) > sourceEngineers.length ? "Синтетически добавленные точки не геокодированы. Пробег показан как оценка." : csvMeta.geocoding.fallbackAddresses ? `${csvMeta.geocoding.fallbackAddresses} из ${csvMeta.geocoding.uniqueAddresses} уникальных адресов CSV имеют резервные координаты. Пробег показан как оценка; процент изменения не рассчитывается.` : extraJobs.some(job => job.geocodeVerified !== true) ? "У срочной заявки не подтверждены координаты. Пробег показан как оценка." : "Часть дорожной матрицы заменена приближённым расстоянием. Пробег показан как оценка.") : result.comparison.reason ?? "Нет сопоставимых назначений для расчёта пробега.";
