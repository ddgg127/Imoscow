...
function AnalyticsView({ result, engineers, distanceReady, distanceWarning, solver, speedKmh, travel }: { result: OptimizationResult; engineers: Engineer[]; distanceReady: boolean; distanceWarning: string; solver: SolverEngine; speedKmh: number; travel?: TravelMatrix }) {
...
export default function Dashboard() {
  ...
  const handleImport = useCallback(async (file: File) => {
    ...
    try {
      ...
      setApplied(null); setPlanResult(null); setReplanChanges([]); setSelectedJobId(null); setSelectedEngineerId(null);
      setDraft(current => ({ ...current, jobs: jobs.length || current.jobs, engineers: imported.engineers?.length || current.engineers }));
