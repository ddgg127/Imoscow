...
function FacetFilters({ groups, selected, onSelected, strict, onStrict, count, total }: { groups: FacetGroup[]; selected: string[]; onSelected: (values: string[]) => void; strict: boolean; onStrict: (value: boolean) => void; count: number; total: number }) {
  return <section className="facet-filters" aria-label="Фильтры"><div className="facet-head"><div><strong>Фильтры</strong><span>Показано {count} из {total}</span></div><label className="facet-strict"><input type="checkbox" checked={strict} onChange={event => onStrict(event.target.checked)} /> Строгий фильтр <small>{strict ? "все выбранные" : "хотя бы один"}</small></label>{selected.length > 0 && <button type="button" onClick={() => onSelected([])}>Сбросить</button>}</div><div className="facet-groups">{groups.map(group => <details key={group.key} className="facet-group"><summary>{group.label}{selected.filter(value => value.startsWith(`${group.key}\u0000`)).length > 0 && <b>{selected.filter(value => value.startsWith(`${group.key}\u0000`)).length}</b>}</summary><div className="facet-options">{group.values.map(value => { const id = `${group.key}\u0000${value}`; return <label key={id}><input type="checkbox" checked={selected.includes(id)} onChange={event => onSelected(event.target.checked ? [...selected, id] : selected.filter(item => item !== id))} /><span>{value}</span></label>; })}</div></details>)}</div>{selected.length > 0 && <p className="facet-selected">{selected.map(value => { const [key, label] = value.split("\u0000"); return <button type="button" key={value} onClick={() => onSelected(selected.filter(item => item !== value))} aria-label={`Убрать фильтр ${label}`}>{groups.find(group => group.key === key)?.label}: {label} ×</button>; })}</p>}</section>;
}
function RequestsView({ jobs, engineers, onOpen, onAdd }: { jobs: Job[]; engineers: Engineer[]; onOpen: (id: string) => void; onAdd: () => void }) {
  const [selected, setSelected] = useState<string[]>([]); const [strict, setStrict] = useState(false); const [showAll, setShowAll] = useState(false);
  const unassigned = jobs.filter(job => !job.engineerId).length;
  const groups: FacetGroup[] = [{ key: "kind", label: "Навык / работа", values: facetOptions(jobs.map(job => job.kind)) }, { key: "equipment", label: "Оборудование", values: facetOptions(jobs.map(job => job.equipment)) }, { key: "transport", label: "Транспорт", values: facetOptions(jobs.map(job => job.requiredTransport)) }, { key: "status", label: "Статус", values: ["В окне", "Нет маршрута"] }];
  const filtered = jobs.filter(job => matchesFacets(selected, strict, { kind: [job.kind], equipment: [job.equipment], transport: [job.requiredTransport], status: [job.engineerId ? "В окне" : "Нет маршрута"] }));
  return <section className="page-view"><div className="view-summary"><article><span>Из CSV и формы</span><strong>{jobs.length}</strong><small>заявок в текущем плане</small></article><article><span>Назначено VRPTW</span><strong>{jobs.length - unassigned}</strong><small>{jobs.length ? ((jobs.length - unassigned) / jobs.length * 100).toFixed(1) : "0.0"}% плана</small></article><article><span>Без назначения</span><strong>{unassigned}</strong><small>нет допустимого окна или ресурса</small></article></div><FacetFilters groups={groups} selected={selected} onSelected={setSelected} strict={strict} onStrict={setStrict} count={filtered.length} total={jobs.length} /><section className="panel queue-panel full-table"><div className="panel-header"><div><h2>Заявки</h2><p>Источник CSV · три региона · {filtered.length} по фильтру</p></div><button className="plain-button" onClick={onAdd}><Plus />Добавить заявку</button></div>{filtered.length ? <JobTable jobs={filtered} engineers={engineers} onOpen={onOpen} limit={showAll ? undefined : 250} /> : <p className="facet-empty">По выбранным условиям заявок нет.</p>}{!showAll && filtered.length > 250 && <button type="button" className="facet-show-all" onClick={() => setShowAll(true)}>Показать все {filtered.length} заявок</button>}</section></section>;
}
function EngineersView({ engineers, result, onOpenDetails, onOpenRoute }: { engineers: Engineer[]; result: OptimizationResult; onOpenDetails: (id: string) => void; onOpenRoute: (id: string) => void }) {
  const [showAll, setShowAll] = useState(false);
  const [selected, setSelected] = useState<string[]>([]); const [strict, setStrict] = useState(false);
  ...
  const filtered = engineers.filter(engineer => matchesFacets(selected, strict, { skills: engineer.skills, equipment: engineer.equipment, transport: [engineer.transport] }));
    ...
    <div className="engineer-card-grid">{shown.map(engineer => {
      ...
      return <article key={engineer.id} className="engineer-card">
        <button type="button" className="engineer-card-main" onClick={() => onOpenDetails(engineer.id)} aria-label={`Навыки и данные: ${engineer.name}`}>
          <div className="engineer-card-head"><span className="avatar large" style={{ background: `${engineer.color}18`, color: engineer.color }}>{engineer.initials}</span><div><strong>{engineer.name}</strong><small>{engineer.region} · {engineer.route}</small></div><b style={{ color: engineer.color }}>{route?.load ?? 0}%</b></div>
...
function EngineerDetailsDialog({ engineer, route, onClose, onOpenRoute }: { engineer: Engineer | null; route?: RoutePlan; onClose: () => void; onOpenRoute: (id: string) => void }) {
  return <Dialog open={Boolean(engineer)} onOpenChange={open => !open && onClose()}>
    <DialogContent className="engineer-details-dialog">
      <DialogHeader>
        ...
        <DialogDescription>{engineer?.region} · {engineer?.route}</DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter><Button variant="outline" onClick={onClose}>Закрыть</Button><Button disabled={!route?.stops.length} onClick={() => { if (engineer) onOpenRoute(engineer.id); }}>{route?.stops.length ? "Открыть маршрут на карте" : "Нет маршрута"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
function AnalyticsView({ result, engineers, distanceReady, distanceWarning }: { result: OptimizationResult; engineers: Engineer[]; distanceReady: boolean; distanceWarning: string }) {
  ...
  return <section className="page-view">
    <div className="view-summary">
      ...
      <article><span>Пробег по планам</span><strong>{formatDistance(result.baseline.distanceKm)} → {formatDistance(result.metrics.distanceKm)}</strong><small>{distanceReady && delta != null ? `${delta < 0 ? "−" : "+"}${Math.abs(delta).toFixed(1).replace(".", ",")}% при одинаковом наборе заявок` : "Процент пока не рассчитывается"}</small></article>
    </div>
    <p className={distanceReady ? "comparison-note ready" : "comparison-note"}>{distanceReady ? "Планы выполняют одинаковые заявки; адреса геокодированы, расстояния рассчитаны по одной дорожной матрице." : distanceWarning}</p>
    <div className="analytics-grid">
      ...
      <article className="panel analytics-panel wide"><div className="panel-header"><div><h2>Сравнение по зонам</h2><p>Оба плана рассчитаны с одинаковыми окнами, ресурсами и расстояниями</p></div></div><div className="distance-bars">{result.zones.map(zone => <div key={zone.name}><span>{zone.name}</span><div className="distance-track"><i className="baseline" style={{ width: `${zone.baselineDistance / maxDistance * 100}%` }} /><i className="optimized" style={{ width: `${zone.distance / maxDistance * 100}%` }} /></div><strong>{zone.baselineAssigned} → {zone.assigned} заявок<br />{zone.baselineEngineers} → {zone.engineers} инженеров<br />{formatDistance(zone.baselineDistance)} → {formatDistance(zone.distance)}</strong></div>)}</div><div className="analytics-legend"><span><i className="baseline" />Последовательный baseline</span><span><i className="optimized" />VRPTW-план</span></div></article>
...
function JobDetailsDialog({ job, engineer, plan, baselineEngineer, baselinePlan, jobs, onClose }: { job: Job | null; engineer?: Engineer; plan?: RoutePlan; baselineEngineer?: Engineer; baselinePlan?: RoutePlan; jobs: Job[]; onClose: () => void }) {
  ...
  return <Dialog open={Boolean(job)} onOpenChange={open => !open && onClose()}>
    <DialogContent className="explain-dialog job-inspect-dialog">
      ...
      {job && <>
        ...
        <div className="job-baseline"><b>Последовательный baseline</b><span>{baselineEngineer && baselinePlan ? `${baselineEngineer.name} · ${baselinePlan.stops.findIndex(stop => stop.jobId === job.id) + 1}-я остановка · ${formatDistance(baselinePlan.distanceKm)} за маршрут` : job.baselineUnassignedReason ?? "Без назначения"}</span></div>
        {engineer && plan ? <>
...
export default function Dashboard() {
  ...
  const result = planResult ?? idleOptimization(activeEngineers, activeJobs, applied?.speedKmh ?? draft.speedKmh, travel);
  const plannedJobs = result.jobs; const visibleJobs = useMemo(() => plannedJobs.filter(job => region === "Все зоны" || job.region === region), [plannedJobs, region]); const baselineJobs = useMemo(() => plannedJobs.filter(job => region === "Все зоны" || job.region === region), [plannedJobs, region]); const selectedJob = plannedJobs.find(job => job.id === selectedJobId) ?? null; const selectedEngineer = activeEngineers.find(item => item.id === selectedJob?.engineerId); const routeByEngineer = useMemo(() => new Map(result.routes.map(route => [route.engineerId, route])), [result.routes]); const visibleEngineers = useMemo(() => activeEngineers.filter(engineer => region === "Все зоны" || engineer.region === region), [activeEngineers, region]); const visibleBaselineRoutes = useMemo(() => result.baselineRoutes.filter(route => { const engineer = activeEngineers.find(item => item.id === route.engineerId); return Boolean(engineer) && (region === "Все зоны" || engineer?.region === region); }), [result.baselineRoutes, activeEngineers, region]);
  ...
  const addUrgent = useCallback(async () => { setFormError(""); if (!urgentForm.address.trim()) { setFormError("Укажите адрес заявки"); return; } const start = Number(urgentForm.start.slice(0, 2)) * 60 + Number(urgentForm.start.slice(3)); const end = Number(urgentForm.end.slice(0, 2)) * 60 + Number(urgentForm.end.slice(3)); if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) { setFormError("Проверьте временное окно"); return; } let coordinates = regionCenters[urgentForm.region]; let geocodeVerified = false; try { const resolved = await new BackendGeocodingProvider("nominatim").geocode(urgentForm.address); if (resolved) { coordinates = resolved; geocodeVerified = true; } } catch { /* fallback */ } const id = `URG-${Date.now().toString().slice(-6)}`; const job: Job = { id, time: `${urgentForm.start}–${urgentForm.end}`, windowStart: start, windowEnd: end, area: urgentForm.region, address: urgentForm.address, kind: urgentForm.kind, tone: "amber", region: urgentForm.region, engineerId: null, baselineEngineerId: null, coordinates, geocodeVerified, risk: false, equipment: urgentForm.equipment, requiredTransport: urgentForm.transport, priority: 10, serviceMinutes: 60, source: "Срочная форма", status: "Новая" }; const nextExtras = [...extraJobs, job]; setExtraJobs(nextExtras); setUrgentOpen(false); setSelectedJobId(id); setUrgentForm(current => ({ ...current, address: "" })); if (applied) { const engineers = scaleEngineers(sourceEngineers, applied.engineers); const merged = composePlanJobs(applied.jobs, applied.windowMinutes, nextExtras).map(item => planResult?.jobs.find(prev => prev.id === item.id) ?? item); void runOptimize(engineers, merged, applied.speedKmh, job.id); } }, [urgentForm, applied, extraJobs, planResult, runOptimize]);
const titles = { plan: ["План работ", started ? "17 августа 2026 · VRPTW по трём регионам" : "Задайте параметры справа и запустите построение маршрутов"], requests: ["Заявки", "CSV и срочные работы"], team: ["Инженеры", "Ресурсы и рассчитанная загрузка"], analytics: ["Аналитика", "Показатели текущего VRPTW-плана"] } as const; const distanceReady = started && result.comparison.sameAssignedJobs && result.comparison.distanceDeltaPercent != null && csvMeta.geocoding.fallbackAddresses === 0 && (applied?.jobs ?? 0) <= sourceJobs.length && (applied?.engineers ?? 0) <= sourceEngineers.length && extraJobs.every(job => job.geocodeVerified === true) && !matrixFallback; const distanceWarning = !started ? "Запустите построение маршрутов для сравнения." : result.comparison.reason ?? ((applied?.jobs ?? 0) > sourceJobs.length || (applied?.engineers ?? 0) > sourceEngineers.length ? "Синтетически добавленные точки не геокодированы. Пробег показан как оценка." : csvMeta.geocoding.fallbackAddresses ? `${csvMeta.geocoding.fallbackAddresses} из ${csvMeta.geocoding.uniqueAddresses} уникальных адресов CSV имеют резервные координаты. Пробег показан как оценка; процент изменения не рассчитывается.` : extraJobs.some(job => job.geocodeVerified !== true) ? "У срочной заявки не подтверждены координаты. Пробег показан как оценка." : matrixFallback ? "Часть дорожной матрицы заменена приближённым расстоянием. Пробег показан как оценка." : "");
...
<section className="metric-grid"><article><div className="metric-head"><span className="metric-icon purple"><Wrench /></span><small>Выполнено заявок</small><Badge className="metric-badge">{started ? `${result.metrics.unassigned} без маршрута` : "не запущено"}</Badge></div><strong>{result.metrics.assigned}<em>/ {result.metrics.total}</em></strong><p>baseline: {result.baseline.assigned} · VRPTW: {result.metrics.assigned}</p></article><article><div className="metric-head"><span className="metric-icon teal"><Route /></span><small>Пробег VRPTW</small><Badge className="metric-badge good">{distanceReady && result.comparison.distanceDeltaPercent != null ? `${result.comparison.distanceDeltaPercent < 0 ? "−" : "+"}${Math.abs(result.comparison.distanceDeltaPercent).toFixed(1).replace(".", ",")}%` : "оценка"}</Badge></div><strong>{result.metrics.distanceKm.toFixed(1).replace(".", ",")}<em> км</em></strong><p>baseline: {formatDistance(result.baseline.distanceKm)}{distanceReady ? "" : " · без сравнения %"}</p></article><article><div className="metric-head"><span className="metric-icon orange"><UsersRound /></span><small>Активные инженеры</small><Badge className="metric-badge">{started ? "baseline → VRPTW" : "ожидание"}</Badge></div><strong>{result.baseline.activeEngineers}<em> → {result.metrics.activeEngineers}</em></strong><p>из {activeEngineers.length} доступных</p></article><article><div className="metric-head"><span className="metric-icon blue"><Database /></span><small>Данные CSV</small><Badge className="metric-badge">{csvMeta.regions.length} зоны</Badge></div><strong>{csvMeta.rows}<em> строк</em></strong><p>координаты найдены: {csvMeta.geocoding.resolvedAddresses}/{csvMeta.geocoding.uniqueAddresses}</p></article></section>
    ...
    {view === "requests" && <RequestsView jobs={plannedJobs} engineers={activeEngineers} onOpen={selectJob} onAdd={() => setUrgentOpen(true)} />}{view === "team" && <EngineersView engineers={activeEngineers} result={result} onOpenDetails={setSelectedEngineerDetailsId} onOpenRoute={openEngineerOnMap} />}{view === "analytics" && <AnalyticsView result={result} engineers={activeEngineers} distanceReady={distanceReady} distanceWarning={distanceWarning} />}</section>
...
<EngineerDetailsDialog engineer={detailsEngineer} route={detailsEngineer ? routeByEngineer.get(detailsEngineer.id) : undefined} onClose={() => setSelectedEngineerDetailsId(null)} onOpenRoute={openEngineerOnMap} />
<JobDetailsDialog job={selectedJob} engineer={selectedEngineer} plan={selectedJob?.engineerId ? routeByEngineer.get(selectedJob.engineerId) : undefined} baselineEngineer={activeEngineers.find(item => item.id === selectedJob?.baselineEngineerId)} baselinePlan={result.baselineRoutes.find(route => route.engineerId === selectedJob?.baselineEngineerId)} jobs={plannedJobs} onClose={() => setSelectedJobId(null)} /></main>;
}
