...
type ViewId = "plan" | "requests" | "team" | "analytics" | "generator";
...
function composePlanJobs(base: Job[], count: number, windowMinutes: number | null, extras: Job[], cancelledIds: string[] = []) {
  ...
  return [...(windowMinutes == null ? scaled : applyAverageWindows(scaled, windowMinutes)), ...extras].map(job => ({ ...job, cancelled: cancelled.has(job.id), status: cancelled.has(job.id) ? "Отменена" : job.status }));
}
...
function AnalyticsView({ result, engineers, distanceReady, distanceWarning, solver, speedKmh, metrics }: { result: OptimizationResult; engineers: Engineer[]; distanceReady: boolean; distanceWarning: string; solver: SolverEngine; speedKmh: number; metrics: ReactNode }) {
...
export default function Dashboard() {
  ...
  const handleImport = useCallback(async (file: File) => {
    ...
    try {
      ...
      setExtraJobs([]); setCancelledJobIds([]); setUnavailableEngineerIds([]); setApplied(null); setPlanResult(null); setTravel(undefined); setMatrixFallback(false); setReplanChanges([]); setSelectedJobId(null); setSelectedEngineerId(null);
      setDraft(current => ({ ...current, jobs: jobs.length, engineers: imported.engineers?.length || sourceEngineers.length, speedKmh: imported.speedKmh ?? current.speedKmh }));
