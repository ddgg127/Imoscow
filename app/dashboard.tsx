...
function JobDetailsDialog({ job, engineer, plan, baselineEngineer, baselinePlan, jobs, onClose, onToggleCancelled }: { job: Job | null; engineer?: Engineer; plan?: RoutePlan; baselineEngineer?: Engineer; baselinePlan?: RoutePlan; jobs: Job[]; onClose: () => void; onToggleCancelled: (id: string) => void }) {
...
export default function Dashboard() {
  ...
  const handleImport = useCallback(async (file: File) => {
    ...
    try {
      ...
      setExtraJobs([]); setCancelledJobIds([]); setUnavailableEngineerIds([]); setApplied(null); setPlanResult(null); setSelectedJobId(null); setSelectedEngineerId(null);
...
<JobDetailsDialog job={selectedJob} engineer={selectedEngineer} plan={selectedJob?.engineerId ? routeByEngineer.get(selectedJob.engineerId) : undefined} baselineEngineer={activeEngineers.find(item => item.id === selectedJob?.baselineEngineerId)} baselinePlan={result.baselineRoutes.find(route => route.engineerId === selectedJob?.baselineEngineerId)} jobs={plannedJobs} onClose={() => setSelectedJobId(null)} onToggleCancelled={toggleJobCancelled} /></main>;
}
