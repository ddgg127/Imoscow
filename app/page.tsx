...
function MapCanvas({ visibleJobs, visibleEngineers, selectedEngineerId, selectedJobId, replanned, compare, onSelectEngineer, onSelectJob, onRoutingState }: { visibleJobs: Job[]; visibleEngineers: Engineer[]; selectedEngineerId: string | null; selectedJobId: string | null; replanned: boolean; compare: boolean; onSelectEngineer: (id: string) => void; onSelectJob: (id: string) => void; onRoutingState: (state: "loading" | "ready" | "fallback") => void }) {
  ...
  useEffect(() => { latestRef.current = { visibleJobs, visibleEngineers, selectedEngineerId, selectedJobId, replanned, compare }; }, [visibleJobs, visibleEngineers, selectedEngineerId, selectedJobId, replanned, compare]);

  useEffect(() => {
    let cancelled = false;
    const provider = new BackendRoutingProvider("osrm");
    void Promise.all(visibleEngineers.map(async engineer => {
      const fallback = routeCoordinates(engineer, visibleJobs, replanned);
      if (fallback.length < 2) return [engineer.id, fallback, true] as const;
      try {
        const result = await provider.buildRoute({ points: fallback, mode: "driving" });
        return [engineer.id, result.geometry.coordinates as Coordinate[], result.provider !== "osrm"] as const;
      } catch { return [engineer.id, fallback, true] as const; }
    })).then(results => {
      if (cancelled) return;
      setRoadRoutes(Object.fromEntries(results.map(([id, coordinates]) => [id, coordinates])));
      const nextState = results.some(([, , fallback]) => fallback) ? "fallback" : "ready";
      setRouteStatus(nextState); onRoutingState(nextState);
    });
    return () => { cancelled = true; };
  }, [visibleEngineers, visibleJobs, replanned, onRoutingState]);
  ...
  return <div className="map-canvas real-map" aria-label="Интерактивная карта маршрутов инженеров"><div ref={containerRef} className="maplibre-host" /><div className="map-tools"><button aria-label="Показать все маршруты" onClick={() => void fitVisible()}><Layers3 size={17} /></button><span /><button aria-label="Увеличить карту" onClick={() => mapRef.current?.zoomIn()}>+</button><button aria-label="Уменьшить карту" onClick={() => mapRef.current?.zoomOut()}>−</button></div><div className={`map-caption ${routeStatus}`}><Navigation size={14} />{routeStatus === "loading" ? "OSRM · строим дорожные маршруты" : routeStatus === "ready" ? "OSRM · маршруты по дорожной сети" : "OSRM недоступен · показана резервная геометрия"}</div></div>;
}

function RequestsView({ onOpen, onAdd }: { onOpen: (id: string) => void; onAdd: () => void }) {
  return <section className="page-view"><div className="view-summary"><article><span>Всего</span><strong>205</strong><small>заявок на 17 августа</small></article><article><span>Назначено</span><strong>203</strong><small>98,5% плана</small></article><article><span>Под риском</span><strong>2</strong><small>нужен контроль диспетчера</small></article></div><section className="panel queue-panel full-table"><div className="panel-header"><div><h2>Все заявки</h2><p>Восток, Юго-восток и Югоцентр</p></div><button className="plain-button" onClick={onAdd}><Plus />Добавить заявку</button></div><div className="job-table"><div className="job-row table-head"><span>Время</span><span>Заявка</span><span>Адрес</span><span>Инженер</span><span>Статус SLA</span><span /></div>{jobs.map(job => { const engineer = engineers.find(item => item.id === job.engineerId)!; return <button className="job-row job-row-button" key={job.id} onClick={() => onOpen(job.id)}><span className="job-time">{job.time}</span><span><i className={`job-tone ${job.tone}`} /><b>№ {job.id}</b><small>{job.kind}</small></span><span><b>{job.area}</b><small>{job.address}</small></span><span className="assigned"><i style={{ background: engineer.color }}>{engineer.initials}</i><b>{engineer.name}</b></span><span><Badge className={job.risk ? "sla risk" : "sla"}>{job.risk ? <AlertTriangle /> : <Clock3 />}{job.risk ? "Риск 14 мин" : "Вовремя"}</Badge></span><span className="row-location"><MapPin /></span></button>; })}</div></section></section>;
}

function EngineersView({ onOpen }: { onOpen: (id: string) => void }) {
  return <section className="page-view"><div className="view-summary"><article><span>Доступно</span><strong>34</strong><small>инженера в трёх зонах</small></article><article><span>На маршрутах</span><strong>31</strong><small>91% команды</small></article><article><span>Средняя загрузка</span><strong>74%</strong><small>без перегрузки смен</small></article></div><div className="engineer-card-grid">{engineers.map(engineer => <button key={engineer.id} className="engineer-card" onClick={() => onOpen(engineer.id)}><div className="engineer-card-head"><span className="avatar large" style={{ background: `${engineer.color}18`, color: engineer.color }}>{engineer.initials}</span><div><strong>{engineer.name}</strong><small>{engineer.region} · {engineer.route}</small></div><b style={{ color: engineer.color }}>{engineer.load}%</b></div><Progress value={engineer.load} className="load-progress" style={{ ["--primary" as string]: engineer.color }} /><div className="engineer-card-meta"><span><b>{engineer.jobs}</b> заявок</span><span><b>{engineer.distance}</b> пробег</span><span><b>3</b> навыка</span></div><small className="open-route">Открыть маршрут на карте</small></button>)}</div></section>;
}

function AnalyticsView() {
  const zones = [{ name: "Восток", jobs: 66, sla: 98, distance: 82 }, { name: "Юго-восток", jobs: 83, sla: 95, distance: 94 }, { name: "Югоцентр", jobs: 56, sla: 97, distance: 72 }];
  return <section className="page-view"><div className="view-summary"><article><span>SLA вовремя</span><strong>97%</strong><small>цель не ниже 95%</small></article><article><span>Экономия пробега</span><strong>18%</strong><small>54,8 км за смену</small></article><article><span>Равномерность загрузки</span><strong>86%</strong><small>разброс снижен на 12%</small></article></div><div className="analytics-grid"><article className="panel analytics-panel"><div className="panel-header"><div><h2>Заявки по зонам</h2><p>205 заявок из контрольного распределения</p></div></div><div className="zone-chart">{zones.map(zone => <div key={zone.name}><span>{zone.name}</span><div><i style={{ width: `${zone.jobs}%` }} /></div><strong>{zone.jobs}</strong></div>)}</div></article><article className="panel analytics-panel"><div className="panel-header"><div><h2>SLA по зонам</h2><p>Доля визитов в пределах окна</p></div></div><div className="sla-chart">{zones.map(zone => <div key={zone.name}><div className="sla-ring" style={{ ["--value" as string]: `${zone.sla * 3.6}deg` }}><strong>{zone.sla}%</strong></div><span>{zone.name}</span></div>)}</div></article><article className="panel analytics-panel wide"><div className="panel-header"><div><h2>Пробег: baseline и план</h2><p>Сокращение после оптимизации маршрутов</p></div></div><div className="distance-bars">{zones.map(zone => <div key={zone.name}><span>{zone.name}</span><div className="distance-track"><i className="baseline" style={{ width: "100%" }} /><i className="optimized" style={{ width: `${zone.distance}%` }} /></div><strong>−{100 - zone.distance}%</strong></div>)}</div><div className="analytics-legend"><span><i className="baseline" />Baseline</span><span><i className="optimized" />Оптимизированный план</span></div></article></div></section>;
}

export default function Home() {
  const [view, setView] = useState<"plan" | "requests" | "team" | "analytics">("plan"); const [region, setRegion] = useState<"Все зоны" | Region>("Все зоны"); const [replanned, setReplanned] = useState(false); const [compare, setCompare] = useState(false); const [urgentOpen, setUrgentOpen] = useState(false); const [selectedJobId, setSelectedJobId] = useState<string | null>(null); const [selectedEngineerId, setSelectedEngineerId] = useState<string | null>(null); const [theme, setTheme] = useState<ThemeId>("light"); const [themeOpen, setThemeOpen] = useState(false); const [routingState, setRoutingState] = useState<"loading" | "ready" | "fallback">("loading");
  ...
  const updateRoutingState = useCallback((state: "loading" | "ready" | "fallback") => setRoutingState(state), []);
  const openEngineerOnMap = useCallback((id: string) => { setSelectedEngineerId(id); setSelectedJobId(null); setView("plan"); }, []);
  const titles = { plan: ["План работ", "Понедельник, 17 августа · Московский регион"], requests: ["Заявки", "Очередь работ и контроль SLA"], team: ["Инженеры", "Загрузка и маршруты команды"], analytics: ["Аналитика", "Эффективность плана по трём зонам"] } as const;
  ...
  return <main className="app-shell"><aside className="sidebar"><Logo /><nav aria-label="Основная навигация"><button className={`nav-item${view === "plan" ? " active" : ""}`} onClick={() => setView("plan")}><Route /><span>Планирование</span></button><button className={`nav-item${view === "requests" ? " active" : ""}`} onClick={() => setView("requests")}><Wrench /><span>Заявки</span><b>205</b></button><button className={`nav-item${view === "team" ? " active" : ""}`} onClick={() => setView("team")}><UsersRound /><span>Инженеры</span></button><button className={`nav-item${view === "analytics" ? " active" : ""}`} onClick={() => setView("analytics")}><BarChart3 /><span>Аналитика</span></button></nav><div className="sidebar-bottom"><button className="nav-item theme-nav" onClick={() => setThemeOpen(open => !open)}><SunMoon /><span>Тема</span></button><button className="nav-item"><CircleHelp /><span>Помощь</span></button><div className="profile"><span>ДК</span><div><strong>Диспетчер</strong><small>В сети</small></div><MoreHorizontal size={17} /></div></div></aside><section className="workspace" id="plan">
    <header className="topbar"><button className="mobile-menu" aria-label="Открыть меню"><Menu /></button><div><h1>{titles[view][0]}</h1><p>{titles[view][1]}</p></div><div className="top-actions"><button className="theme-button" onClick={() => setThemeOpen(open => !open)}><Contrast /><span>{themes.find(item => item.id === theme)?.name}</span><ChevronDown /></button><button className="search"><Search /><span>Найти заявку</span><kbd>⌘ K</kbd></button><button className="icon-button" aria-label="Уведомления"><Bell /><i /></button><Button className="urgent-button" onClick={() => setUrgentOpen(true)}><Zap />Срочная заявка</Button></div></header>
    ...
    {view === "requests" && <RequestsView onOpen={selectJob} onAdd={() => setUrgentOpen(true)} />}
    {view === "team" && <EngineersView onOpen={openEngineerOnMap} />}
