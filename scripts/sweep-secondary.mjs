...
function place(engineer, route, job, openExtraKm) {
  ...
  for (let index = 0; index <= route.length; index++) {
    ...
    if (!plan) continue;
    const opened = route.length === 0;
    const score = (plan.distanceKm - before) * DISTANCE_WEIGHT + (opened ? VEHICLE_COST + openExtraKm * DISTANCE_WEIGHT : 0);
    if (!best || score < best.score) best = { route: next, score };
  }
  return best;
}

function regret(engineers, jobs, openExtraKm, soloGapKm) {
  const routes = new Map(engineers.map(engineer => [engineer.id, []]));
  const open = [...jobs];
  const solo = soloGapKm * DISTANCE_WEIGHT;
  while (open.length) {
    let pick = null;
    for (let jobIndex = 0; jobIndex < open.length; jobIndex++) {
      ...
      for (const engineer of engineers) {
        ...
        const placed = place(engineer, routes.get(engineer.id), job, openExtraKm);
        if (placed) costs.push({ engineer, ...placed });
      }
      if (!costs.length) continue;
      costs.sort((a, b) => a.score - b.score);
      const gap = (costs[1]?.score ?? costs[0].score + solo) - costs[0].score;
  ...
  for (const job of jobs) {
    ...
    for (const from of engineers) {
      ...
      for (let index = 0; index < current.length; index++) {
        ...
        const withJob = place(from, without, job, openExtraKm);
        if (!withJob) continue;
        for (const to of engineers) {
          ...
          const back = place(to, destination, displaced, openExtraKm);
          if (!back) continue;
...
const openExtras = [0, 2, 4, 6, 8, 10, 14, 20, 30, 40];
const soloGaps = [0, 4, 8, 14, 24];
const rows = [];
for (const openExtraKm of openExtras) {
  for (const soloGapKm of soloGaps) {
    const total = { openExtraKm, soloGapKm, assigned: 0, urgent: 0, fleet: 0, km: 0 };
    for (const set of sets) {
      const result = measure(set.engineers, set.jobs, regret(set.engineers, set.jobs, openExtraKm, soloGapKm));
    ...
    rows.push(total);
    console.log(`extra ${openExtraKm}\tgap ${soloGapKm}\t${total.assigned}\turgent ${total.urgent}\tfleet ${total.fleet}\tkm ${total.km.toFixed(1)}`);
  }
}

rows.sort((a, b) => b.assigned - a.assigned || b.urgent - a.urgent || a.fleet - b.fleet || a.km - b.km);
console.log("BEST");
for (const row of rows.slice(0, 8)) console.log(`extra ${row.openExtraKm} gap ${row.soloGapKm}  ${row.assigned} urgent ${row.urgent} fleet ${row.fleet} km ${row.km.toFixed(1)}`);
