  ...
  return <section className="page-view data-editor-view">
    <div className="panel sheet-panel">
      ...
      <div className="sheet-scroll" role="region" aria-label={`Редактор ${entity === "jobs" ? "заявок" : "инженеров"}`} tabIndex={0}>
        {entity === "jobs" ? <table className="data-sheet"><thead><tr><th>№</th><th>ID</th><th>Адрес</th><th>Район</th><th>Регион</th><th>Координаты</th><th>Окно с</th><th>Окно до</th><th>Норматив, мин</th><th>Навык</th><th>Вид работ</th><th>Оборудование</th><th>Транспорт</th><th>Допустимый транспорт</th><th>Приоритет</th><th>Выполнение</th><th>Отменена</th><th>Источник</th></tr></thead><tbody>{shown.map(({ item, index }) => { const job = item as Job; const label = `Заявка ${job.id}`; return <tr key={index}><th scope="row">{index + 1}</th><td><Cell label={`${label}: ID`} value={job.id} onChange={id => updateJob(index, { id })} /></td><td><Cell label={`${label}: адрес`} value={job.address} onChange={address => updateJob(index, { address })} className="sheet-wide" /></td><td><Cell label={`${label}: район`} value={job.area} onChange={area => updateJob(index, { area })} /></td><td><Choice label={`${label}: регион`} value={job.region} options={regions} onChange={region => updateJob(index, { region: region as Region })} /></td><td><Coordinates label={label} point={job.coordinates} onChange={coordinates => updateJob(index, { coordinates, geocodeVerified: false, geocodeQuality: "fallback" })} /></td><td><Cell label={`${label}: начало окна`} type="time" value={timeInput(job.windowStart)} onChange={value => updateJob(index, { windowStart: parseTime(value), time: `${value}–${timeInput(job.windowEnd)}` })} /></td><td><Cell label={`${label}: конец окна`} type="time" value={timeInput(job.windowEnd)} onChange={value => updateJob(index, { windowEnd: parseTime(value), time: `${timeInput(job.windowStart)}–${value}` })} /></td><td><Cell label={`${label}: норматив`} type="number" min={1} max={1440} value={job.serviceMinutes} onChange={value => updateJob(index, { serviceMinutes: Number(value) })} /></td><td><Choice label={`${label}: навык`} value={job.kind} options={skills} onChange={kind => updateJob(index, { kind })} /></td><td><Cell label={`${label}: вид работ`} value={job.workType ?? ""} onChange={workType => updateJob(index, { workType })} /></td><td><Cell label={`${label}: оборудование`} value={job.equipment} onChange={equipment => updateJob(index, { equipment })} /></td><td><Choice label={`${label}: транспорт`} value={job.requiredTransport} options={transports} onChange={requiredTransport => updateJob(index, { requiredTransport })} /></td><td><textarea aria-label={`${label}: допустимый транспорт через запятую`} key={(job.allowedTransports ?? []).join(",")} defaultValue={(job.allowedTransports ?? []).join(", ")} rows={2} onBlur={event => { const options = list(event.target.value); updateJob(index, { allowedTransports: options.length ? options : undefined }); }} /></td><td><Cell label={`${label}: приоритет`} type="number" min={1} max={100} value={job.priority} onChange={value => updateJob(index, { priority: Number(value) })} /></td><td><select aria-label={`${label}: выполнение`} value={executionAtTime(job, stopsByJob.get(job.id), simTime)} onChange={event => updateJob(index, { executionStatus: event.target.value as Job["executionStatus"] })}>{statuses.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></td><td><input type="checkbox" aria-label={`${label}: отменена`} checked={Boolean(job.cancelled)} onChange={event => updateJob(index, { cancelled: event.target.checked })} /></td><td><Cell label={`${label}: источник`} value={job.source} onChange={source => updateJob(index, { source })} /></td></tr>; })}</tbody></table>
