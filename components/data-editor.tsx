    ...
    <div className="panel sheet-panel">
      ...
      <div className="sheet-scroll" role="region" aria-label={`Редактор ${entity === "jobs" ? "заявок" : "инженеров"}`} tabIndex={0}>
        {entity === "jobs" ? <table className="data-sheet"><thead><tr><th>№</th><th>ID</th><th>Адрес</th><th>Район</th><th>Регион</th><th>Координаты</th><th>Окно с</th><th>Окно до</th><th>Работа, мин</th><th>Норматив</th><th>Резерв дороги</th><th>Расчёт дороги</th><th>Источник нормы</th><th>Навык</th><th>Вид работ</th><th>Оборудование</th><th>Транспорт</th><th>Явные альтернативы</th><th>Срочность</th><th>Бизнес-приоритет</th><th>Состояние</th><th>Отменена</th><th>Источник</th></tr></thead><tbody>{shown.map(({ item, index }) => { const job = item as Job; const label = `Заявка ${job.id}`; return <tr key={index}>
          ...
          <td><select aria-label={`${label}: срочность`} value={job.urgency ?? "normal"} onChange={event => updateJob(index, { urgency: event.target.value as Job["urgency"] })}><option value="normal">Обычная</option><option value="urgent">Срочная</option></select></td>
          <td><Cell label={`${label}: бизнес-приоритет`} type="number" min={1} max={100} value={job.priority} onChange={value => updateJob(index, { priority: Number(value) })} /></td>
          <td><select aria-label={`${label}: состояние`} value={executionAtTime(job, stopsByJob.get(job.id), simTime)} onChange={event => updateJob(index, { executionStatus: event.target.value as Job["executionStatus"] })}>{statuses.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></td>
