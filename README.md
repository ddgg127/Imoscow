...
5. при перепланировании мягко учитывать прежнего инженера и время клиента вместе с числом задействованных инженеров и дорожным пробегом.
...
Вес каждого уровня до покрытия и бизнес-приоритета строго больше максимально возможной суммы нижестоящих расходов. Поэтому сокращение пробега или сохранение назначений не может оправдать потерю допустимой заявки. Штрафы стабильности, число инженеров и пробег на последнем уровне сопоставляются по суммарной стоимости.
...
Для стабильного показа можно подключить собственные OSRM-серверы переменными `OSRM_CAR_URL`, `OSRM_FOOT_URL`, `OSRM_BIKE_URL` (базовый URL до `/route/v1/driving` и `/table/v1/driving`). Они проверяются первыми; публичные OSM-серверы остаются резервом. Если серверный прокси не получает матрицу, браузер пробует публичный OSRM напрямую. Перед показом запустите `npm run check:routing` в той же среде, где работает веб-сервис: команда проверит ответы Route и Table для каждого заданного и публичного адреса. Без доступного дорожного сервера расчёт остаётся оценочным, дорожные линии скрываются, а точный процент экономии пробега не показывается. В репозитории нет записанной дорожной матрицы, поэтому офлайн-демонстрация **не подтверждает** реальный пробег.
