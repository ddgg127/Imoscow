...
7. Независимо рассчитывается детерминированный baseline на том же наборе данных и той же матрице.
...
| Транспорт | Жёсткое | Тип транспорта заявки и инженера должен совпадать |
...
- Серверный solver ограничен временем расчёта; при недоступности сервиса включается явно обозначенный эвристический fallback.
- Baseline детерминированный и не выполняет глобальный поиск, но использует наиболее дешёвую допустимую вставку. Для буквального конкурсного baseline «первый доступный инженер, добавление в конец» нужен отдельный режим.
...
7. **4:20–5:00 — результат и воспроизводимость.** Экспортировать CSV/JSON, кратко показать архитектуру и назвать ограничения прототипа и следующий шаг — проверку 24 адресов и производственное развёртывание API.
