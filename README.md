...
Для стабильного показа можно подключить собственные OSRM-серверы переменными `OSRM_CAR_URL`, `OSRM_FOOT_URL`, `OSRM_BIKE_URL` (базовый URL до `/route/v1/driving` и `/table/v1/driving`). Они проверяются первыми; публичные OSM-серверы остаются резервом. Если серверный прокси не получает матрицу, браузер пробует публичный OSRM напрямую. Перед показом запустите `npm run check:routing` в той же среде, где работает веб-сервис: команда проверит ответы Route и Table для каждого заданного и публичного адреса. Без доступного дорожного сервера расчёт остаётся оценочным; карта показывает пунктирные прямые между точками с плавной оценочной анимацией, а точный процент экономии пробега не показывается. В репозитории нет записанной дорожной матрицы, поэтому офлайн-демонстрация **не подтверждает** реальный пробег.
...
- Дорожные линии запрашиваются сначала через API приложения, затем при его отказе напрямую из браузера у публичных OSM-маршрутизаторов. Пеший и велосипедный профили берутся у FOSSGIS; для общественного транспорта при недоступном мультимодальном графе показывается **явно помеченная пешеходная оценка**, не фактический маршрут транспорта. При отказе всех источников карта показывает пунктирную прямую между последовательными точками и плавное оценочное перемещение; эта линия не является дорожным маршрутом. Матрица пробега по-прежнему зависит от OSRM и при её отказе помечается оценочной.
