...
- Дорожные линии запрашиваются через API приложения и при его отказе напрямую из браузера у публичных OSM-маршрутизаторов. Запросы независимых инженеров идут параллельно с ограничением, готовые линии кэшируются и появляются без ожидания остальных. Через 45 секунд ожидание завершается; недостающие линии можно повторно запросить кнопкой на карте без пересчёта плана. Пеший и велосипедный профили берутся у FOSSGIS; для общественного транспорта при недоступном мультимодальном графе показывается **явно помеченный пешеходный путь**, не фактический маршрут транспорта. Если все источники недоступны, маршрут инженера остаётся невидимым, а маркер не перемещается по выдуманной прямой. Публичные сервисы не гарантируют получения всех линий; для гарантии нужен собственный маршрутизатор. Матрица пробега по-прежнему зависит от OSRM и при её отказе помечается оценочной.
