...
- Данные графа получены из [OpenStreetMap](https://www.openstreetmap.org/copyright) и распространяются по ODbL. Для обновления нужны Python с `osmium` и OSM PBF-выгрузки Москвы и Московской области; генератор `scripts/build-road-graph.py` принимает исходные `.osm.pbf`, повторяемые `--bbox` для обслуживаемых районов и `--arterial-bbox` для связывающих трасс. Московское расширение строится с `--bbox 37.34,55.58,37.85,55.93`, затем объединяется с графом удалённых точек через `scripts/merge-road-graphs.py`. После изменения географии обслуживания граф нужно пересобрать и проверить на новых координатах. Ручная интеграционная проверка четырёх наборов: `node scripts/qa-bench.mjs` при запущенном solver на порту 8008.
